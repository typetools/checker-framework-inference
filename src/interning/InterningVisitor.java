        ...
        if (id instanceof JCTree.JCAnnotatedType) {
            ...
            for (JCAnnotation jcann : jcat.getAnnotations()) {
                ...
                if (realChecker.INTERNED.getAnnotationType().equals(cl)) {
                    ...
                    // We could add an option that would relax this rule and not forbid it if it's
                    // common enough. The point is to issue an error if we see thigns like "new
                    // @Interned String("foo")" which is obviously not interned.
                ...
                new Heuristics.Matcher() {
                    ...
                    public Boolean visitBinary(BinaryTree tree, Void p) {
                        if (tree.getKind() == Tree.Kind.EQUAL_TO) { // a.compareTo(b) == 0
                            // looking for a.compareTo(b) or b.compareTo(a)
                            ExpressionTree leftTree = tree.getLeftOperand();
                            // looking for 0
                            ExpressionTree rightTree = tree.getRightOperand();
                            ...
                            // looking for a==b
                            ExpressionTree leftTree = tree.getLeftOperand();
                            // looking for a.compareTo(b) == 0 or b.compareTo(a) == 0
                            ExpressionTree rightTree = tree.getRightOperand();
                            if (leftTree != node) {
                                return false;
                            }
