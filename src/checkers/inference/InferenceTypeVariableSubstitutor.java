 ...
 *
 * <p>Assuming we have a method:
 *
 * <p>{@code <@0 T extends @1 Object> (@2) T method((@3) T t) {}}
 *
 * <p>Where the return type is annotated as: {@code <(@2 | 0) T extends (@2 | 1) Object>}
 *
 * <p>And the argument {@code t} is annotated as: {@code <(@3 | 0) T extends (@3 | 1) Object>}
 *
 * <p>Given the following use of method:
 *
 * <p>{@code @4 String arg;}
 *
 * <p>{@code this.<@5 String>method(arg);}
 *
 * <p>We want the substitutor to return a method of type:
 *
 * <p>{@code (@2 | @5) String method((@3 | @5) String t)}
 *
 * <p>In other words, this method states:
 *
 * <p>if (@2 exists) then
 *
 * <p>the return type has @2 as a primary annotation
 *
 * <p>else
 *
 * <p>the return type has @5 as a primary annotation
 *
 * <p>if (@3 exists) then
 *
 * <p>the parameter has @3 as a primary annotation
 *
 * <p>else
 *
 * <p>the return type has @5 as a primary annotation
...
public class InferenceTypeVariableSubstitutor extends TypeVariableSubstitutor {
    ...
    public InferenceTypeVariableSubstitutor(
    ...
    protected AnnotatedTypeMirror substituteTypeVariable(
        ...
        if (!useUpperBound.getAnnotations().isEmpty()) {
            ...
            if (upperBoundSlot instanceof ExistentialVariableSlot) {
                // the type of the use may already have an existential variable inserted for its
                // declaration we remove it (because it's between the potential variable and the
                // bounds) and replace it with one that is between the SAME potential variable but
                // the argumenht instead

                final VariableSlot potentialSlot =
                        ((ExistentialVariableSlot) upperBoundSlot).getPotentialSlot();
                ...
                if (argument.getKind() != TypeKind.TYPEVAR) {
                    ...
                    if (alternative != null) {
                        final ExistentialVariableSlot slot =
                                new ExistentialVariableSlot(
                                        slotManager.nextId(), potentialSlot, alternative);
            ...
            if (!types.isSameType(use.getUnderlyingType(), argument.getUnderlyingType())) {
                ...
                if (!InferenceMain.isHackMode()) {
                    throw new BugInCF(
                            "Expected ExistentialTypeVariable to substitute:\n"
                                    + "use="
                                    + use
                                    + "\n"
                                    + "argument="
