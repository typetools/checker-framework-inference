        ...
        if ( !useUpperBound.getAnnotations().isEmpty()) {
            ...
            if (upperBoundSlot instanceof ExistentialVariableSlot) {
                ...
                if (argument.getKind() != TypeKind.TYPEVAR) {
                    ...
                    if (alternative != null) {
                        final ExistentialVariableSlot slot = new ExistentialVariableSlot(slotManager.nextId(), potentialSlot, alternative);
                        argument.replaceAnnotation(slotManager.getAnnotation(slot));
                        slotManager.addVariable(slot);
                    } else {
                        if (!InferenceMain.isHackMode()) {
                            throw new CheckerFrameworkBug("Null alternative: " + argument + ", use=" + use);
                        }
                    }
                } else {
            ...
            if (!types.isSameType(use.getUnderlyingType(), argument.getUnderlyingType())) {
                ...
                if (!InferenceMain.isHackMode()) {
                    throw new CheckerFrameworkBug("Expected ExistentialTypeVariable to substitute:\n"
                                    + "use=" + use + "\n"
                                    + "argument=" + argument + "\n"
                    );
                }
