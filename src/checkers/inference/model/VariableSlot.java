 ...
 * After the Solver is run, each VariableSlot should have an assigned value which is then written to
 * the output Jaif file for later reinsertion into the original source code.
 *
 * <p>Before the Solver is run, VariableSlots are represented by @VarAnnot( slot id ) annotations on
 * AnnotatedTypeMirrors. When an AnnotatedTypeMirror is encountered in a position that would
 ...
 *
 * <p>Variable slot hold references to slots it is refined by, and slots it is merged to.
 */
public class VariableSlot extends Slot {
    ...
    /**
     * Should this VariableSlot be inserted back into the source code. This should be false for
     * types have have an implicit annotation and slots for pre-annotated code.
     */
    ...
    private final Set<RefinementVariableSlot> refinedToSlots =
            new HashSet<RefinementVariableSlot>();
