 ...
 *
 * <p>Note: We DON'T put the existential variable in the primary annotation location of type
 * variables. if we do this, all of the ExistentialVariableSlots will be overridden by the one
 * primary annotation. Instead, we traverse the bounds and add them there
 *
 * <p>We go through and remove the potentialVariable from all of these locations and only replace
 * them if they are NOT in non-defaultable locations
 *
 * <p>Also, any type variables we encounter in the declaration should already be fully annotated
 * possibly with ExistentialVariables already, so no need to kick-off the existential variable
 * inserter recursively
 *
 * <p>An example appears below:
 *
 * <p>Assume we have a potentialVariable p, an AnnotatedTypeVariable corresponding to the
 * declaration of a type parameter T, and an AnnotatedTypeVariable corresponding to a use of T:
 *
 * <p>{@code p := @VarAnnot(3)}
 *
 * <p>{@code typeParam := <@VarAnnot(1) T extends @VarAnnot(2) Object>}
 *
 * <p>{@code useOfTypeParam := <@VarAnnot(1) T extends @VarAnnot(2) Object> // corresponds to
 * declaration T t;}
 *
 * <p>Recall from the class comments of ExistentialVariable that we use the shorthand: (@3 | @1) -
 * to indicate an Existential variable that states: if (@3 exists) use @3 else use @1
 *
 * <p>After a call to {@code existentialVariableInserter.insert(p, useOfTypeParam, typeParam)} we
 * want useOfTypeParam := <(@3 | @1) T extends (@3 | @2) object>
 *
 * <p>However, recall that to represent (@3 | @1) we create an ExistentialVariable which is
 * represented by a VarAnnot. E.g. (@4 (@3 | @1) represents an ExistentialVariable with an id of 4
 * which states: if (@3 exists) use @3 else use @1
 *
 * <p>Using the above shorthand, a call to {@code existentialVariableInserter.insert(p,
 * useOfTypeParam, typeParam)} will result in the following state:
 *
 * <p>{@code Let e1 and e2 be instances of existential variable.}
 *
 * <p>{@code Let e1 = (@4 (@3 | @1)) // see ExistentialVariable for the meaning of this notation}
 *
 * <p>{@code Let e2 = (@5 (@3 | @2))}
 *
 * <p>State after call to insert:
 *
 * <p>{@code e1 := @VarAnnot(4)}
 *
 * <p>{@code e2 := @VarAnnot(5)}
 *
 * <p>{@code p := @VarAnnot(3)}
 *
 * <p>{@code typeParam := <@VarAnnot(1) T extends @VarAnnot(2) Object>}
 *
 * <p>{@code useOfTypeParam := <@VarAnnot(4) T extends @VarAnnot(5) Object>}
 */
public class ExistentialVariableInserter {
    ...
    public ExistentialVariableInserter(
        ...
        // bottom is used to force an annotation to exist in a non-defaultable location if it was
        // written explicitly
    ...
    public void insert(
            ...
            final AnnotatedTypeMirror typeUse,
    ...
    public void insert(
        ...
        // propagates the potentialVariable in all of the locations that will be replaced by an
        // ExistentialVariable
        ...
        // now remove only the primary (which has already been propagated to the bounds by
        // fixUpBoundAnnotations)
        typeUse.removeAnnotation(potentialVarAnno);

        final InsertionVisitor insertionVisitor =
                new InsertionVisitor(potentialVariable, potentialVarAnno, mustExist);
    ...
    private class InsertionVisitor extends EquivalentAtmComboScanner<Void, Void> {
        ...
        public void matchAndReplacePrimary(
                final AnnotatedTypeMirror typeUse, final AnnotatedTypeMirror declaration) {
            ...
            if (slotManager.getVariableSlot(typeUse).equals(potentialVariable)) {
                ...
                if (declSlot instanceof VariableSlot) {
                    ...
                    final ExistentialVariableSlot existVar =
                            varAnnotator.getOrCreateExistentialVariable(
                                    typeUse, potentialVariable, varSlot);
        ...
        protected String defaultErrorMessage(
                org.checkerframework.framework.type.AnnotatedTypeMirror type1,
                org.checkerframework.framework.type.AnnotatedTypeMirror type2,
                Void aVoid) {
        ...
        @Override
        protected Void scanWithNull(
                AnnotatedTypeMirror type1, AnnotatedTypeMirror type2, Void aVoid) {
            throw new BugInCF(
                    "Encounter null type in ExistentialVariableInserter:\n"
                            + "type1="
                            + type1
                            + "\n"
                            + "type2="
        ...
        public Void visitArray_Array(
                AnnotatedArrayType typeUse, AnnotatedArrayType declaration, Void aVoid) {
            ...
            // component types will not have the potentialVarAnno on them, so instead copy over
            // other annotations
        ...
        public Void visitDeclared_Declared(
                AnnotatedDeclaredType typeUse, AnnotatedDeclaredType declaration, Void v) {
            if (visited.contains(typeUse, declaration)) {
                return null;
            }
            ...
            // Once we reach a Declared type, only the primary annotation of that declared type can
            // possibly
            // contain the potentialVarAnno, so instead we copy over the other annotations from the
            // declared type
            // to the typeUse
            matchAndReplacePrimary(typeUse, declaration);

            final Iterator<AnnotatedTypeMirror> typeUseArgs = typeUse.getTypeArguments().iterator();
            final Iterator<AnnotatedTypeMirror> declArgs =
                    declaration.getTypeArguments().iterator();
            ...
            while (typeUseArgs.hasNext() && declArgs.hasNext()) {
                ...
                if (nextUse
                        != nextDecl) { // these two can be the same when a recursive type parameter
                    // uses
            ...
            if (typeUseArgs.hasNext() || declArgs.hasNext()) {
                throw new BugInCF(
                        "Mismatched number of type arguments for types:\n"
                                + "typeUse="
                                + typeUse
                                + "\n"
                                + "declaration="
        ...
        public Void visitIntersection_Intersection(
                AnnotatedIntersectionType typeUse, AnnotatedIntersectionType declaration, Void v) {
            if (visited.contains(typeUse, declaration)) {
                return null;
            }
            ...
            // This might change if we actually define the semantics of primary annotations on
            // AnnotatedIntersectionTypes
            matchAndReplacePrimary(typeUse, declaration);
        ...
        // in ExistentialVariableInserter.insert will cause it to be applied as a primary annotation
        // to E.
        // We want this to occur in order to propagate the @PotentialVar, but we don't want ANY
        // primary
        // annotations on type variable when we are done or the ExistentialVariableSlots will be
        // overwritten
