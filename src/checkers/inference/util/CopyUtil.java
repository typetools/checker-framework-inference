    ...
    private static void copyAnnotationsImpl(
        ...
        if (fromKind == PACKAGE) {
            ...
            if (fromSuperTypes.size() != toSuperTypes.size()) {
                throw new BugInCF(
                        "unequal super types when copying INTERSECTION type! from: "
                                + fromIntersec
                                + " to: "
                                + toIntersec);
            }
            ...
            // only check equality from one side to the other side since the Map sizes are asserted
            // to be equal.
            ...
            for (Entry<DeclaredType, AnnotatedDeclaredType> entry : fromSuperTypesMap.entrySet()) {
                copyAnnotationsImpl(
                        entry.getValue(), toSuperTypesMap.get(entry.getKey()), copyMethod, visited);
            }
        } else {
            throw new BugInCF(
                    "InferenceUtils.copyAnnotationsImpl: unhandled getKind results: "
                            + from
                            + " and "
                            + to
                            + "\n    of kinds: "
