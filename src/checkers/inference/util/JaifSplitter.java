 ...
 *
 * <p>Splitting jaifs also has the following effects:
 *
 * <p>a) The AFU doesn't have to search as many Java files when searching for the correct file to
 * insert into.
 *
 * <p>b) If the AFU crashes on a malformed JAIF for one package, annotations are still inserted into
 * the other packages
 *
 * <p>c) The shell script can and will report when it has completed a package and how many total
 * packages there are. Therefore, you can get some sense of progress when inserting the annotations
 * for a large project
 *
 * <p>To use JaifSplitter from source code, create a new JaifSplitter and call jaifSplitter.split()
 *
 * <p>To use JaifSplitter from the command-line, run scripts/splitJaif or scripts/debugSplitJaif
 */
