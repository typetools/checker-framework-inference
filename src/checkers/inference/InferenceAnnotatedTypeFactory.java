 ...
 *
 * <p>Adding annotations is accomplished through three means:
 *
 * <p>1. If we have the source code for a particular type, the InferenceTreeAnnotator and the
 * VariableAnnotator will add the relevant annotation (either VarAnnot or constant real annotation)
 * based on the type's corresponding tree and the rules of the InferrableChecker. If the
 * InferrableChecker determines that a value is constant then the realAnnotatedTypeFactory is
 * consulted to get this value. see {@link
 * checkers.inference.InferenceAnnotatedTypeFactory#annotateImplicit(com.sun.source.tree.Tree,
 * checkers.types.AnnotatedTypeMirror)}
 *
 * <p>2. If we do NOT have the source code then the realAnnotatedTypeFactory is used to determine a
 * constant value to place on the given "library", i.e. from bytecode, type. see {@link
 ...
 * VariableAnnotator#storeElementType. If these elements are encountered again, the annotations from
 * the stored type are copied to the annotations of the type being annotated. see {@link
 * checkers.inference.InferenceAnnotatedTypeFactory#annotateImplicit(javax.lang.model.element.Element,
 * checkers.types.AnnotatedTypeMirror)}
 *
 * <p>Note: a number of constraints are created by members of this class see {@link
 * checkers.inference.InferenceQualifierHierarchy} and {@link
 * checkers.inference.InferenceTypeHierarchy}
 *
 * <p>Finally, Variables are created while flow is being performed (every time getAnnotatedType is
 * called on a class tree that hasn't yet been annotated). Constraints are created after flow has
 * been for a given class when the visitor requests types.
 */
public class InferenceAnnotatedTypeFactory extends BaseAnnotatedTypeFactory {
    ...
    // number of Java files you are compiling, you can use this number to gauge progress of
    // inference.
    // See setRoot below
    ...
    public InferenceAnnotatedTypeFactory(
        ...
        existentialInserter =
                new ExistentialVariableInserter(
                        slotManager, constraintManager, unqualified, varAnnot, variableAnnotator);

        inferencePoly =
                new InferenceQualifierPolymorphism(slotManager, variableAnnotator, varAnnot);
        postInit();
    }
    ...
    protected CFAnalysis createFlowAnalysis(List<Pair<VariableElement, CFValue>> fieldValues) {
        return realChecker.createInferenceAnalysis(
                inferenceChecker, this, fieldValues, slotManager, constraintManager, realChecker);
    }
    ...
    public ConstantToVariableAnnotator getNewConstantToVariableAnnotator() {
        return new ConstantToVariableAnnotator(
                unqualified, varAnnot, variableAnnotator, slotManager);
    }
     ...
     *
     * <p>{@code postDirectSuperTypes( @9 ArrayList< @7 String>, List( @4 List<@7 String>, @0 Object
     * ) )}
     *
     * <p>we would like supertypes to become:
     *
     * <p>{@code List( @9 List<@7 String>, @9 Object )}
     *
    ...
    protected void postDirectSuperTypes(
        ...
        // annotations from type with those for the declaration of the super type
    ...
    public void postAsMemberOf(
        ...
        if (withCombineConstraints) {
            ...
            final CombVariableSlot combSlot =
                    new CombVariableSlot(null, slotManager.nextId(), recvSlot, declSlot);
    ...
    /**
     * TODO: The implementation in AnnotatedTypeFactory essentially replaces the parameterized
     * bounds with concrete bounds (e.g. {@code <T extends @Nullable Object, E extends T>} => {@code
     * <T extends @Nullable Object, E extends @Nullable Object>} TO CORRECTLY MODEL THE RESULTING
     * CONSTRAINTS WOULD WE NOT DO THE SAME THING? OR CREATE A COMBVAR FOR THAT LOCATION?
     */
    @Override
    public List<AnnotatedTypeParameterBounds> typeVariablesFromUse(
            final AnnotatedDeclaredType useType, final TypeElement element) {
        ...
        final List<AnnotatedTypeMirror> declaredTypeParameters =
                ownerOfTypeParams.getTypeArguments();

        final List<AnnotatedTypeParameterBounds> result = new ArrayList<>();

        for (int i = 0; i < declaredTypeParameters.size(); ++i) {
            final AnnotatedTypeVariable declaredTypeParam =
                    (AnnotatedTypeVariable) declaredTypeParameters.get(i);
            result.add(
                    new AnnotatedTypeParameterBounds(
                            declaredTypeParam.getUpperBound(), declaredTypeParam.getLowerBound()));

            // TODO: Original InferenceAnnotatedTypeFactory#typeVariablesFromUse would create a
            // combine constraint between the useType and the effectiveUpperBound of the
            // declaredTypeParameter and then copy the annotations from the type with the CombVars
     ...
     *
     * <p>TODO: This is essentially the default implementation of AnnotatedTypeFactory.methodFromUse
     * with a space to later add comb constraints. One difference is how the receiver is gotten.
     * Perhaps we should just change getSelfType? But I am not sure where getSelfType is used yet
     *
    ...
    public ParameterizedMethodType methodFromUse(final MethodInvocationTree methodInvocationTree) {
        ...
        // added it. It is also what the AnnotatedTypeFactory default implementation does
        final AnnotatedExecutableType methodOfReceiver =
                AnnotatedTypes.asMemberOf(types, this, receiverType, methodElem);
        ParameterizedMethodType mType =
     ...
     * aside from comb constraints, track down the differences with constructorFromUse
     *
     * <p>Note: super() and this() calls
     *
     * @see
    ...
    public ParameterizedMethodType constructorFromUse(final NewClassTree newClassTree) {
        ...
        ParameterizedMethodType substitutedPair =
    ...
    private <EXP_TREE extends ExpressionTree> ParameterizedMethodType substituteTypeArgs(
        ...
        final Map<TypeVariable, AnnotatedTypeMirror> typeVarMapping =
                AnnotatedTypes.findTypeArguments(
                        processingEnv, this, expressionTree, methodElement, methodType);
     ...
     * TODO: Expand
     *
     * <p>If we have a cached AnnotatedTypeMirror for the element then copy its annotations to type
     * else if we can get the source tree for the declaration of that element visit it with the tree
     * annotator else get the AnnotatedTypeMirror from the real AnnotatedTypeFactory and copy its
     * annotations to type
     *
