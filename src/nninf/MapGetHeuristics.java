 ...
 *       <pre><code>assert map.containsKey(key); Object v = map.get(key);</code></pre>
 *       Or
 *       <pre><code>assert map.get(key) != null; Object v = map.get(key);</code></pre>
 *   <li value="4">Preceded by an check of contains or nullness if test that throws an exception, in
 *       the first line:
 *       <pre><code>if (!map.contains(key)) throw new Exception(); Object v = map.get(key);</code>
 *       </pre>
 *   <li value="5">Preceded by a put-if-absent pattern convention:
 *       <pre>
 *       <code>if (!map.contains(key)) map.put(key, DEFAULT_VALUE); Object v = map.get(key);</code>
 *       </pre>
 * </ol>
 */
